...
- 2026-06-18: BUILD STEP 1 of the hybrid redesign DONE (branch `claude/crosswordle-hybrid-redesign-2hwi01`): per-word Wordle history board. Tapping a word now shows a clean horizontal board below the grid — one colored row per past guess + a live input row — solving Laura's "hard to read the word inside the crossing grid" complaint. Interpretation chosen: kept the crossword grid as the spatial map and ADDED the board (didn't replace the grid); both reflect the same state. New persisted state `state.attempts` (per-word guess history) with a `??=` migration in loadState. No empty padding rows / no 6-row cap shown yet — that lands with the guess-cap work (step 2), so the board grows with guesses for now. Grid height budget trimmed (0.46→0.40) and `--wb` cell var added in sizeCells to make room. Files: game.js (renderWordBoard/boardRow + finalizeGuess records history), index.html (#word-board), styles.css (.wb-* ). No browser available in env — verified via node syntax check, validate-puzzles, and an isolated logic harness; NOT yet visually QA'd in a real browser.
- 2026-06-18: BUILD STEP 2 DONE (branch `claude/crosswordle-hybrid-redesign-2hwi01`): per-word 6-guess cap + lose-ability + opt-in reveal. New state: `failed[]`, `revealed[]`, `ended` (with `??=` migrations; `won` now means "ended with every word solved", `ended` means "every word solved-or-failed"). Hitting MAX_GUESSES (6) without solving marks the word `failed` and auto-advances to the next OPEN word — the puzzle does NOT end on a single fail. Game ends (`onEnd`/`showEndModal`) only when `allWordsClosed()`. `checkSolvedWords` now skips failed words so crossings can't silently convert a fail into a solve. Failed words show a "Reveal word" button (word-bar, accent color); `revealWord()` locks the word's cells via the existing `lockCell` mechanic, which seeds ONLY the shared crossing letters to neighbors (neighbors still solved manually). Board (step 1) now pads to 6 rows while a word is open. End modal handles win ("Solved! 🎉") vs partial ("Good try! 🧩 · N/M solved · K fails"). PROVISIONAL scoring call to revisit in step 5: stats `recordWin`/streak only fire on a full win (no fails) — a puzzle with fails counts as completed-but-not-won. Verified with a headless DOM-stub harness (/tmp/harness.js, not committed) that loads the real game.js and drives win/fail/reveal via keystrokes — all pass. Still NOT visually QA'd in a real browser.
- 2026-06-18: WORD-FIT CURATION PRINCIPLE (recurring issue — this is the founding frustration). Puzzle words must be PROTOTYPICAL/evocative of the theme, not merely technically members. Apply the test: "if someone said this theme, would a player actually name this word?" Known offender classes: (a) ingredients standing in for the category (SUGAR/SYRUP in food-dish themes), (b) non-prototypical members (FACE in Body Parts — Laura's original example), (c) category contradictions (SUN in "Night Sky"). Fixes applied 2026-06-18: SUGAR→SUNDAE (Sweet Treats), FACE→FOOT (Body Parts), and "Night Sky"→"Up in the Sky" (re-theme: SUN/MOON/STAR/COMET all fit "in the sky"; couldn't swap SUN itself — its crossing is locked to the S_N pattern by STAR×MOON, so a word swap would need a layout redesign). Still-borderline, left as-is pending Laura's call: SYRUP in Breakfast (an ingredient but strongly evokes breakfast), FENCE/EARTH in In the Garden, SCOUT in Camping Trip. When the tagged catalog (later step) lands, each word should carry theme-family tags AND a prototypicality signal so generation only picks evocative members. ALWAYS run `node scripts/validate-puzzles.cjs` after any word/layout change.
